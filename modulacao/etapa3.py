...
def erro_por_snr(clean_signal, original_bits, isNZR):
    
    snr=0
    erro_snr = {'Quantidade de erros': [], 'SNR': []}
    ...
    while True:
        ...
        if (isNZR):
            decode = decode_nrz(noisy_signal, len(original_bits))
        # else:
        #     decode = decode_manchester(noisy_signal, len(original_bits))
            
        quant_erros = 0
        
        for i in range(0, len(original_bits)):
            if (original_bits[i] != decode[i]): quant_erros+=1
            
        erro_snr['Quantidade de erros'].append(quant_erros)
        erro_snr['SNR'].append(-snr)
        print(f'\nQuantidade de erro: {quant_erros}\nSNR: {snr}')
        if (quant_erros == len(original_bits) or snr < -100): 
            break
...
original_bits = "10001001110101000111"
erro_snr_nzr = erro_por_snr(encode_nrz(original_bits), original_bits, True)

plt.scatter(x = erro_snr_nzr['SNR'], y = erro_snr_nzr['Quantidade de erros'])